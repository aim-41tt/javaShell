...
public class MonitorCommand implements Command {
	...
	private void appendSystemInfo(StringBuilder sb) {
		...
		sb.append("Информация о системе:\n").append("  ОС: ").append(osBean.getName()).append(", ")
				.append(osBean.getVersion()).append("\n").append("  Архитектура: ").append(osBean.getArch())
				.append("\n").append("  Количество процессоров (ядер): ").append(threadBean.getThreadCount() - 1)
	...
	private void appendCpuUsage(StringBuilder sb) {
		...
		sb.append("Использование ЦП:\n").append("  Загрузка ЦП системы: ").append(formatPercentage(systemCpuLoad))
				.append(" %\n").append("  Загрузка ЦП процесса: ").append(formatPercentage(processCpuLoad))
				.append(" %\n\n");
	}
	...
	private void appendMemoryUsage(StringBuilder sb) {
		...
		sb.append("Использование памяти:\n").append("  Использовано памяти: ").append(bytesToMB(usedMemory))
